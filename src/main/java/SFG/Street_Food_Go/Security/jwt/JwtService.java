package SFG.Street_Food_Go.Security.jwt;
...
public class JwtService {
    ...
    public String issue(final String subject, final Collection<String> roles) {
        ...
        return Jwts.builder()
            .setSubject(subject) // Αντί για .subject()
            .setIssuer(this.issuer) // Αντί για .issuer()
            .setAudience(this.audience) // Αντί για .audience()
            .claim("roles", roles)
    ...
    public Claims parse(final String token) {
        return Jwts.parserBuilder() // απο parser το εκανα parserBuilder
